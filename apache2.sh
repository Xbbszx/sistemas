        ...
        if [[ -z $1 ]]; then
                ...
                if [[ "$1" == "--help" ]]; then
                        ...
                        if [[ $# !=  1 ]]; then
                                ...
                                echo "<VirtualHost *:80>" >> midominio.conf
                                echo "ServerName Ejemplo" >> midominio.conf
                                echo "ServerAlias Ejemplo" >> midominio.conf
                                echo "DocumentRoot /var/www/config" >> midominio.conf
                                echo "DirectoryIndex index.html" >> midominio.conf
                                echo "</VirtualHost>" >> midominio.conf
                        ...
                        if [[ $# !=  1 ]]; then
                                ...
                                echo "5. Ver la configuración"
                                echo "6. Reiniciar la configuración"
                                echo "7. Para salir"
                                while true
                                do
                                read -p "Escribe en número para elegir la opción: " opcion
                                        if [[ $opcion == "1" ]]; then
                                                ...
                                                cat /etc/apache2/sites-available/midominio.conf
                                        elif [[ $opcion == "6" ]]; then
                                                sudo touch midominio.conf
                                                echo "<VirtualHost *:80>" >> midominio.conf
                                                echo "ServerName Ejemplo" >> midominio.conf
